...
class PlanarIK:
    ...
    def physical_to_abstract_joints(self, abstract_joints):
        ...
        zeros = np.tile(np.array([0, pi / 2 + self.j1_offset, 0, self.j3_offset, 0, pi + self.j5_offset, pi / 4]),
                        (2, 1))
        # Add abstract joints accounting for different directions
        physical_joints = zeros + np.array(
            [[0, -abstract_joints[0, 0], 0, abstract_joints[0, 1], 0, abstract_joints[0, 2], 0],
    ...
    def rrr_abstract_ik(self, target):
        ...
        # for wrist
        oc_x = x_e - a3 * np.cos(phi)
        oc_y = y_e - a3 * np.sin(phi)

        theta_2_1 = math.acos((oc_x ** 2 + oc_y ** 2 - a1 ** 2 - a2 ** 2) / (2 * a1 * a2))
        theta_2_2 = -math.acos((oc_x ** 2 + oc_y ** 2 - a1 ** 2 - a2 ** 2) / (2 * a1 * a2))

        theta_1_1 = math.atan2(oc_y, oc_x) - math.atan2(a2 * np.sin(theta_2_1), a1 + a2 * np.cos(theta_2_1))
        theta_1_2 = math.atan2(oc_y, oc_x) - math.atan2(a2 * np.sin(theta_2_2), a1 + a2 * np.cos(theta_2_2))
